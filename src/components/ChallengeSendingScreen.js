...
const ChallengeSendingScreen = () => {
          ...
          const unsubscribe = onSnapshot(challengeRef, async (snapshot) => {
            ...
            if (challenge.status === "accepted") {
              try {
                ...
                unsubscribe();
                navigate(
                  `/Quiz2/${docRef.id}?sender=${challenge.senderId}&receiver=${challenge.receiverId}`
                );
              } catch (error) {
  ...
  const handleDifficultyChange = (difficulty) => {
    ...
    if (
      ...
      courseData[selectedSubject][difficulty]
    ) {
  ...
  const userList = users.map((user) => (
    <li
      ...
      style={
        selectedUserId === user.id ? styles.selectedUserItem : styles.userItem
      }
    >
      ...
      <div style={styles.scrollableContainer}>
        <div style={styles.content}>
          {loading ? (
            ...
            <div style={styles.noDataContainer}>
              <p style={styles.noDataMessage}>
                Sorry, no users available for your program of study at the
          ...
          <div style={styles.modalContent}>
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              <div style={styles.scrollableList}>
                {Object.keys(courseData).filter(
                  (subject) =>
                    courseData[subject].programOfStudy ===
                      userData.programOfStudy &&
                    courseData[subject].semesterOfStudy ===
                      userData.semesterOfStudy &&
                    courseData[subject].levelOfStudy === userData.levelOfStudy
                ).length > 0 ? (
                  Object.keys(courseData)
                    .filter(
                      (subject) =>
                        ...
                        courseData[subject].semesterOfStudy ===
                          userData.semesterOfStudy &&
                        courseData[subject].levelOfStudy ===
                  ...
                  <p style={styles.noDataMessage}>
                    No courses available now. Please come back later.
                  </p>
                )}
              </div>
            </div>
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              >
                ...
                {selectedSubject &&
                  Object.keys(courseData[selectedSubject])
                    .filter(
                      (key) =>
                        ![
                          "programOfStudy",
            ...
            >
              {isLoading ? (
                <span style={styles.loadingText}>
                  Please wait while challenge is accepted...
                </span>
              ) : (
                "Send Challenge"
              )}

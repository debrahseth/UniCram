          ...
          if (question.type === "Multiple Answers") {
            ...
            if (question.type === "Fill-in" && Array.isArray(question.answer)) {
              ...
              if (rangeAnswer && rangeAnswer.includes("to")) {
                ...
                isCorrect =
                  !isNaN(userNumber) && userNumber >= min && userNumber <= max;
                displayCorrectAnswer = isCorrect
                  ? userAnswer
                  : `${rangeAnswer}`;
              } else {
...
const styles = {
  container: {
    ...
    overflow: "hidden",
    fontFamily:
      "'Poppins', 'Roboto', 'Open Sans', 'Helvetica Neue', Arial, sans-serif",
  },
  background: {
    content: '""',
    position: "absolute",
